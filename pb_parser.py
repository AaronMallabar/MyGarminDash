...
def get_max_power_peaks(powers, times):
    # Returns a dict of best powers for [5, 30, 60, 120, 300, 600, 1200, 1800, 3600]
    windows = [5, 30, 60, 120, 300, 600, 1200, 1800, 3600]
    bests = {str(w): 0 for w in windows}
    ...
    for w in windows:
        ...
        bests[str(w)] = round(max_avg)
    return bests

def get_fastest_paces(dists, times):
    # Dists in meters, Times in ms. Target dists: 1mi(1609.34), 5k(5000), 5mi(8046.72)
    targets = {'1mi': 1609.34, '5k': 5000, '5mi': 8046.72}
    bests = {k: None for k in targets.keys()}
        ...
        for right in range(len(dists)):
            while dists[right] - dists[left] >= target_m:
                ...
                if time_diff < min_time:
                    min_time = time_diff
                left += 1
        bests[k] = round(min_time / 1000) if min_time != float('inf') else None
    return bests
...
def pb_parse_activity_details(details):
    ...
    power_bests = get_max_power_peaks(powers, ptimes)
    pace_bests = get_fastest_paces(dists, dtimes)
